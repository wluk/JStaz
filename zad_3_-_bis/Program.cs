...
namespace zad_3___bis
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            InsertCourse();
            //InsertData();
            //var Users = session.Query<Person>().Count();
            var Depts = session.Query<Department>().ToList();
            foreach (var d in Depts)
            {
                var CoursysInfo = CoursesInfoByDepartment(d.DepartmentID);
                Console.WriteLine("\n\nOddzial: " + d.Name);
                foreach (var c in CoursysInfo)
                {
                    Console.WriteLine("Nazwa kursu: {0}", c.Title);
                    if (c.OnlineCourse != null)
                    {
                        Console.WriteLine("e-kurs\nAdres strony z kursem: {1}", c.OnlineCourse.URL);
                    }
                    if (c.OnsiteCourse != null)
                    {
                        Console.WriteLine("Szkolenie stacjonarne\nCzas trwana kursu: {1}", c.OnsiteCourse.Time);
                    }
                }
            }
        ...
        //            var c4 = new Course
        //            {
        //                Title = "Kurs wypełniania dokumentacji unijnej",
        //                DepartmentID = d2,
        //                Credits = 777
        //            };
        //            session.Save(c4);
        ...
        private static List<Course> CoursesInfoByDepartment(int DepId)
        {
            ...
            var q = session.Query<Course>().Where(c => c.DepartmentID.DepartmentID == DepId).ToList();
            return session.Query<Course>().Where(c => c.DepartmentID.DepartmentID == DepId).ToList();
        }
    }
}